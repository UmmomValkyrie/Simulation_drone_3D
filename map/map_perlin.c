...
extern float fps_cam_x, fps_cam_y, fps_cam_z;
...
void generate_chunk(chunk* chunk, int cx, int cy) {
    ...
    for (int y = 0; y < CHUNK_SIZE; y++) {
        for (int x = 0; x < CHUNK_SIZE; x++) {
            ...
            currentChunk->heightmap[x][y] = perlin2D(worldX, worldY);
        }
    }
}
void updateCurrentChunk(void) {
    ...
    if (cx != lastChunkX || cy != lastChunkY) {
        if (currentChunk == NULL) currentChunk = malloc(sizeof(chunk));
        if (!load_chunk(currentChunk, cx, cy)) {
            generate_chunk(currentChunk, cx, cy);
...
void save_chunk(chunk* chunk) {
    char path[64];
    sprintf(path, "chunks/chunk_%d_%d.bin", chunk->chunkX, chunk->chunkY);
...
int load_chunk(chunk* chunk, int cx, int cy) {
    char path[64];
    sprintf(path, "chunks/chunk_%d_%d.bin", cx, cy);
...
void generate_heightmap() {
    for (int y = 0; y < CHUNK_SIZE; y++) {
        for (int x = 0; x < CHUNK_SIZE; x++) {
            ...
            currentChunk->heightmap[x][y] = noise;
        }
    }
}
...
//     float neighbor;
//     // N (y+1)
//     if (y + 1 >= CHUNK_SIZE || currentChunk->heightmap[x][y + 1] * AMPLITUDE < h_top) {
...
//     }
//     // S (y-1)
//     if (y == 0 || currentChunk->heightmap[x][y - 1] * AMPLITUDE < h_top) {
//         glColor3f(0.15f, 0.15f, 0.15f);
//         glBegin(GL_QUADS);
//         glVertex3f(x,     y, h_bottom);
//         glVertex3f(x + 1, y, h_bottom);
...
//     }
//     // E (x+1)
//     if (x + 1 >= CHUNK_SIZE || currentChunk->heightmap[x + 1][y] * AMPLITUDE < h_top) {
...
//     }
//     // O (x-1)
//     if (x == 0 || currentChunk->heightmap[x - 1][y] * AMPLITUDE < h_top) {
//         glColor3f(0.2f, 0.2f, 0.2f);
//         glBegin(GL_QUADS);
//         glVertex3f(x, y,     h_bottom);
//         glVertex3f(x, y + 1, h_bottom);
...
//     for (int y = 0; y < CHUNK_SIZE; y++) {
//         for (int x = 0; x < CHUNK_SIZE; x++) {
//             draw_terrain_cube(x, y, currentChunk->heightmap[x][y]);
//         }
//     }
// }
void draw_terrain_smooth() {
    for (int y = 0; y < CHUNK_SIZE - 1; y++) {
        for (int x = 0; x < CHUNK_SIZE - 1; x++) {
            float h1 = currentChunk->heightmap[x][y] * AMPLITUDE;
            float h2 = currentChunk->heightmap[x+1][y] * AMPLITUDE;
            float h3 = currentChunk->heightmap[x][y+1] * AMPLITUDE;
            float h4 = currentChunk->heightmap[x+1][y+1] * AMPLITUDE;

            glBegin(GL_TRIANGLES);

            set_color(currentChunk->heightmap[x][y]); glVertex3f(x, y, h1);
            set_color(currentChunk->heightmap[x+1][y]); glVertex3f(x + 1, y, h2);
            set_color(currentChunk->heightmap[x][y+1]); glVertex3f(x, y + 1, h3);

            set_color(currentChunk->heightmap[x+1][y]); glVertex3f(x + 1, y, h2);
            set_color(currentChunk->heightmap[x+1][y+1]); glVertex3f(x + 1, y + 1, h4);
            set_color(currentChunk->heightmap[x][y+1]); glVertex3f(x, y + 1, h3);
...
void draw_forest() {
    for (int i = 0; i < NB_TREES; i++) {
        int x = forest[i].x;
        int y = forest[i].y;
        float height_tree = forest[i].height_tree;
        float h = currentChunk->heightmap[x][y];

        if( h > WATER_LEVEL && h < MOUNTAIN_LEVEL){
            h = h * AMPLITUDE;
            glPushMatrix();
    
            glTranslatef(x + 0.5f, y + 0.2f, h - 0.1f);  // Position de base du tronc
...
void draw_rock() {
    for(int i = 0; i < NB_ROCKS; i++){
        ...
        if( h > WATER_LEVEL && h > MOUNTAIN_LEVEL){
            ...
            glTranslatef(x + 0.5f, y + 0.2f, h - 0.1f);  
            
            GLUquadric* quad = gluNewQuadric();
            glColor3f(rocks_place[i].r, rocks_place[i].r - 0.05f, rocks_place[i].r - 0.1f);
...
void draw_water(float level, float time) {
    time = time / 3600.0f;
    float waterHeight = level * AMPLITUDE;  // axe vertical = Z
    ...
    for (int y = 0; y < CHUNK_SIZE - 1; y++) {
        for (int x = 0; x < CHUNK_SIZE - 1; x++) {
            if (currentChunk->heightmap[x][y] < level || currentChunk->heightmap[x+1][y] < level ||
                currentChunk->heightmap[x][y+1] < level || currentChunk->heightmap[x+1][y+1] < level) {
...
float get_ground_height_at(float x, float y) {
    ...
    if (xi < 0 || xi >= CHUNK_SIZE - 1 || yi < 0 || yi >= CHUNK_SIZE - 1)
        return 0.0f;

    float dx = x - xi;
    float dy = y - yi;

    float h00 = currentChunk->heightmap[xi][yi] * AMPLITUDE;
    float h10 = currentChunk->heightmap[xi + 1][yi] * AMPLITUDE;
    float h01 = currentChunk->heightmap[xi][yi + 1] * AMPLITUDE;
    float h11 = currentChunk->heightmap[xi + 1][yi + 1] * AMPLITUDE;

    float h;

    // Déterminons dans quel triangle on est
    if (dx + dy < 1.0f) {
        // Triangle inférieur gauche : (0,0)-(1,0)-(0,1)
        // Interpolation barycentrique
        h = h00 * (1 - dx - dy) + h10 * dx + h01 * dy;
    } else {
        // Triangle supérieur droit : (1,0)-(1,1)-(0,1)
        float dx1 = 1 - dx;
        float dy1 = 1 - dy;
...
if (TTF_Init() < 0) {
    ...
    TTF_Font* font = TTF_OpenFont("./arial.ttf", 24);  // Assure-toi que le fichier existe
    if (!font) {
    ...
    while (running) {
        ...
        drawText(100, 140, "1. Générer un nouveau chunk central (0,0)", font, white);
        drawText(100, 180, "2. Charger chunk existant (0,0)", font, white);
    ...
    if (!currentChunk)
        currentChunk = malloc(sizeof(chunk));

    if (choix == 1) {
        generate_chunk(currentChunk, 0, 0);
        save_chunk(currentChunk);
        generate_forest();
        generate_rock();
        generate_clouds();
    } else if (choix == 2) {
        if (!load_chunk(currentChunk, 0, 0)) {
            generate_chunk(currentChunk, 0, 0);
            save_chunk(currentChunk);
            generate_forest();
            generate_rock();
...
void affichage(const DroneState* drone, float camRadius, float camPitch, float camYaw) {
    ...
    if (mode_fps) {
        ...
        Vec3 up = {0.0f, 0.0f, 1.0f}; // caméra FPS = verticale fixe
